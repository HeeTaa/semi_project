...
<div class="gab"></div>
	...
	<form id="FoodmodifyForm"
		action="${pageContext.request.contextPath}/FoodmodifyProc.foodbo"
		method="post">
...
<footer>
	...
	// 추천 버튼 클릭했을 때 이벤트 처리
		...
		btn_recommend.addEventListener("click",function(){
			...
			location.href = "${pageContext.request.contextPath}/FoodRecommendProc.foodbo?board_seq=${dto.getBoard_seq()}&currentPage=${currentPage}";
		});	
	...
	$(document).ready(function(){
		...
		btn_reportSubmit.addEventListener("click",function(){
			location.href="${pageContext.request.contextPath}/FoodreportProc.foodrp?board_seq=${dto.getBoard_seq()}&currentPage=${currentPage}&report_reason="+$("#report_reason").val(); // get 방식으로 넘기기
	...
	btn_cancel.addEventListener("click",function(){
		location.href = "${pageContext.request.contextPath}/toFoodDetailView.foodbo?board_seq=${dto.getBoard_seq()}&currentPage=${currentPage}";
	})
	...
	$(document).ready(function(){
		...
		$("#btnSubmitCmt").on("click",function(){
			...
			$("#comment_content").val("");
			$.ajax({
				url : "${pageContext.request.contextPath}/FoodinsertProc.foodco",
				type : "post",
				data : data
			}).done(function(rs){
			 if(rs =="success"){
		...
		function getCommentList(){
			...
			$.ajax({
				type: "get",
				url : "${pageContext.request.contextPath}/getFoodCommentProc.foodco?board_seq=${dto.getBoard_seq()}",
				dataType : "json"
			}).done(function(data){
				 $("#cmt_box").empty();
				 console.log("456");
		...
		$(document).on("click",".btn-modifyCmtSuc",function(e){
			...
			$.ajax({
				url:"${pageContext.request.contextPath}/FoodmodifyProc.foodco?comment_seq=" + $(e.target).val() + "&comment_content=" + comment_content ,
				type:"get"
			}).done(function(rs){
				if(rs=="success"){
					getCommentList();
		...
		$(document).on("click","#btn_commentDelete",function(e){
			...
			$.ajax({
				url:"${pageContext.request.contextPath}/FooddeleteProc.foodco?comment_seq=" + $(e.target).val(),
				type : 'get'
			}).done(function(rs){
				if(rs=="success"){
