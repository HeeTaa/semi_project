...
<div class="container ">
	<div class="row">
		<div class="col">
			<h1 class="display-4 main_copy text_gradation">고민게시글 상세조회  </h1>
...
<div class="gab"></div>
	...
	<form id="modifyForm"
		action="${pageContext.request.contextPath}/modifyProc.freebo"
		method="post">
			...
			<div class="col">
				<table class="table table-bordered">
					...
					<tr>
						...
						<!-- "${pageContext.request.contextPath}/${fileDTO.getPicAddr()}/${fileDTO.getSystem_name()}" -->
						<div>
						<c:if test="${!empty fileDTO.getSystem_name()}">
							<img class="detailImg" alt="안나오는 이미지입니다." src="${pageContext.request.contextPath}/${fileDTO.getPicAddr()}/${fileDTO.getSystem_name()}">
						</c:if>
						</div>
						</div>
...
<footer>
	...
	// 추천 버튼 클릭했을 때 이벤트 처리
		...
		btn_recommend.addEventListener("click",function(){
			...
			location.href = "${pageContext.request.contextPath}/RecommendProc.freebo?board_seq=${dto.getBoard_seq()}&currentPage=${currentPage}";
		});	
	...
	$(document).ready(function(){
		...
		btn_reportSubmit.addEventListener("click",function(){
			location.href="${pageContext.request.contextPath}/reportProc.freerp?board_seq=${dto.getBoard_seq()}&currentPage=${currentPage}&report_reason="+$("#report_reason").val(); // get 방식으로 넘기기
	...
	btn_cancel.addEventListener("click",function(){
		location.href = "${pageContext.request.contextPath}/toDetailView.freebo?board_seq=${dto.getBoard_seq()}&currentPage=${currentPage}";
	})
	...
	$(document).ready(function(){
		...
		$("#btnSubmitCmt").on("click",function(){
			...
			$("#comment_content").val("");
			$.ajax({
				url : "${pageContext.request.contextPath}/insertProc.freeco",
				type : "post",
				data : data
			}).done(function(rs){
			 if(rs =="success"){
		...
		function getCommentList(){
			...
			$.ajax({
				type: "get",
				url : "${pageContext.request.contextPath}/getCommentProc.freeco?board_seq=${dto.getBoard_seq()}",
				dataType : "json"
			}).done(function(data){
				 $("#cmt_box").empty();
				 console.log("456");
		...
		$(document).on("click",".btn-modifyCmtSuc",function(e){
			...
			$.ajax({
				url:"${pageContext.request.contextPath}/modifyProc.freeco?comment_seq=" + $(e.target).val() + "&comment_content=" + comment_content ,
				type:"get"
			}).done(function(rs){
				if(rs=="success"){
					getCommentList();
		...
		let origin_content; // 취소 버튼 눌렀을 때 수정 전 값이 저장되어있는 변수  
		...
		$(document).on("click",".btn-modifyCmtCan",function(e){
 		...
 		// val(빈 값) -> 현재 가지고 있는 값
		// val(값) -> val이 () 안에 있는 값으로 지정됨
		
		// 댓글 삭제 버튼 클릭 시 이벤트 처리
		$(document).on("click","#btn_commentDelete",function(e){
			console.log("댓글 삭제 버튼 클릭");
			$.ajax({
				url:"${pageContext.request.contextPath}/deleteProc.freeco?comment_seq=" + $(e.target).val(),
				type : 'get'
			}).done(function(rs){
				if(rs=="success"){
